      ...
      {comments.length === 0 ? (
        ...
        <ul className="space-y-3">
          {[...comments].reverse().map((c) => {
            ...
            return (
              <li key={c.id} className="bg-muted/60 border border-border rounded-xl p-4">
                ...
                {bodyHtml ? (
                  <div
                    className="adf-content text-sm text-muted-foreground leading-relaxed"
                    dangerouslySetInnerHTML={{ __html: bodyHtml }}
                  />
                ) : (
                  <p className="text-sm text-muted-foreground italic">Empty comment.</p>
                )}
              </li>
            );
          })}
