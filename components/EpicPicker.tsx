...
function ChevronDown() {
  return (
    <svg className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400" width="14" height="14" viewBox="0 0 14 14" fill="none">
...
export default function EpicPicker() {
  ...
  useEffect(() => {
    ...
    fetch("/api/jira/projects")
      ...
      .then((data: JiraProject[]) => setProjects(data))
  ...
  useEffect(() => {
    ...
    fetch(`/api/jira/epics?project=${encodeURIComponent(selectedProject)}`)
      ...
      .then((data: JiraIssue[]) => setEpics(data))
      .catch((e: Error) => setError(e.message))
